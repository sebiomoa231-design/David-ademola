...
import { LiveMultimodalStudio } from "@/components/creative-suite/LiveMultimodalStudio";
import { WebsiteBuilderLive } from "@/components/creative-suite/WebsiteBuilderLive";
import { FilesViewLive } from "@/components/creative-suite/FilesViewLive";
import { davidTemplates, type DavidTemplate } from "@/lib/david-templates";
...
const navGroups: { label: string; items: { route: RouteKey; label: string; icon: LucideIcon; badge?: string }[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { route: "templates", label: "Templates", icon: Library },
...
const routeMeta: Record<RouteKey, { eyebrow: string; title: string; description: string }> = {
  "operating-system": { eyebrow: "David AI / Operating system", title: "David is listening.", description: "A voice-first model environment for conversation, capability routing, execution, and verified results." },
  ...
  chat: { eyebrow: "David AI / Conversation", title: "What should we move forward today?", description: "Ask a question, route a goal, or start a creative production workflow." },
  projects: { eyebrow: "David AI / Work systems", title: "Projects with momentum.", description: "Keep goals, tasks, files, and model runs connected in one operating view." },
  ...
  agents: { eyebrow: "David AI / Command center", title: "A coordinated capability system.", description: "Route research, strategy, creative, engineering, and quality-control work through focused capabilities." },
  ...
  templates: { eyebrow: "David AI / Work systems", title: "Reusable systems that run for real.", description: "Start a governed workflow from an accessible template, then review the live execution state and evidence." },
...
export default function DavidCommandCenter({ initialRoute = "dashboard" }: { initialRoute?: string }) {
  ...
  const voice = useVoiceOS({
    onResult: (result) => {
      ...
      setToast({ kind: "success", text: result.agentsUsed.length ? `David coordinated ${result.agentsUsed.length} capability path${result.agentsUsed.length === 1 ? "" : "s"}.` : "David completed the voice response." });
    },
  ...
  const submitMessage = async (value = draft) => {
    ...
    try {
      ...
      const delegated = response.agents_used?.length ? `capability path · ${response.agents_used.join(", ")}` : "live response";
    ...
    <div className={cx("david-shell", activeRoute === "operating-system" && "david-shell-os", preferences.reducedMotion && "david-pref-reduced-motion", preferences.highContrast && "david-pref-high-contrast", preferences.quietMode && "david-pref-quiet")}>
      ...
      <main className="david-main">
        ...
        <div className="page-frame">
          ...
          {activeRoute === "templates" && <TemplatesView onRunObjective={(objective) => void runObjective(objective)} notify={setToast} />}
          {activeRoute === "creative" && <CreativeView onNavigate={navigate} notify={setToast} />}
          {activeRoute === "website-builder" && <WebsiteBuilderLive notify={setToast} />}
          {activeRoute === "video-studio" && <VideoStudio notify={setToast} />}
          {activeRoute === "voice-studio" && <LiveMultimodalStudio kind="voice" notify={setToast} />}
          {activeRoute === "image-studio" && <LiveMultimodalStudio kind="image" notify={setToast} />}
          {activeRoute === "music-studio" && <LiveMultimodalStudio kind="music" notify={setToast} />}
          {activeRoute === "enhance-studio" && <LiveMultimodalStudio kind="enhance" notify={setToast} />}
          {activeRoute === "edit-studio" && <LiveMultimodalStudio kind="edit" notify={setToast} />}
          {activeRoute === "reshoot-studio" && <LiveMultimodalStudio kind="reshoot" notify={setToast} />}
          {activeRoute === "files" && <FilesViewLive notify={setToast} />}
...
function OperatingSystemView({ voice, connected, preferences, onPreferenceChange, onNavigate }: { voice: ReturnType<typeof useVoiceOS>; connected: boolean; preferences: DavidSettings; onPreferenceChange: (key: DavidPreferenceKey, value: boolean) => void; onNavigate: (route: RouteKey) => void }) {
  ...
  return <div className={cx("os-view", `os-state-${voice.state}`, isExecuting && "os-executing", voice.response && "os-has-response")}>
    ...
    <div className="os-view-header"><div><span className="micro-label">DAVID AI / VOICE-FIRST OPERATING SYSTEM</span><h1>Command the system naturally.</h1><p>Speak a goal. David interprets it, routes governed capabilities, and returns an audible, reviewable result.</p></div><div className="os-header-status"><span className="status-dot" />{connected ? "NETWORK CONNECTED" : "CONNECTION UNAVAILABLE"}</div></div>
    ...
    <div className="os-controls"><button className="button button-primary" onClick={voice.state === "speaking" || voice.state === "generating_audio" ? voice.cancel : voice.toggle} disabled={voice.state === "thinking"}><Mic size={16} />{voice.state === "listening" ? "Stop & process" : voice.state === "speaking" || voice.state === "generating_audio" ? "Stop voice output" : "Talk to David"}</button><button className="button button-secondary" onClick={voice.isPaused ? voice.resume : voice.pause} disabled={!voice.isSpeaking}><Pause size={16} />{voice.isPaused ? "Resume" : "Pause"}</button><button className="button button-secondary" onClick={() => void voice.replay()} disabled={!voice.response}><Play size={16} />Replay</button><button className="button button-secondary" onClick={voice.cancel} disabled={voice.state === "idle"}><X size={16} />Cancel</button><button className="button button-secondary" onClick={() => onNavigate("agents")}><Bot size={16} />Open model tools</button><button className="button button-secondary" onClick={() => onNavigate("chat")}><MessageSquare size={16} />Text fallback</button></div>
...
function MemoryView({ notify }: { notify: (toast: Toast) => void }) {
  ...
  useEffect(() => {
    let mounted = true;
    api.memories().then((items) => mounted && setMemories(items)).catch(() => mounted && notify({ kind: "error", text: "Live memory is unavailable. David did not substitute sample knowledge cards." })).finally(() => mounted && setLoading(false));
    return () => { mounted = false; };
  }, [notify]);
  ...
  async function search() {
    ...
    try {
      ...
      notify({ kind: "error", text: "Memory search is unavailable from the connected backend." });
    } finally {
      setSearching(false);
    }
  }
  ...
  async function addMemory() {
    const content = window.prompt("What should David remember?");
    if (!content?.trim() || working) return;
    setWorking(true);
    try {
      const memory = await api.addMemory({ content: content.trim(), type: "user_note", source: "david_os" });
      setMemories((current) => [memory, ...current]);
      notify({ kind: "success", text: "Memory saved to the live workspace." });
    } catch {
      notify({ kind: "error", text: "David could not save this memory because the live memory service is unavailable." });
    } finally {
      setWorking(false);
    }
  }

  async function removeMemory(memory: { id?: string; content: string }) {
    if (!memory.id) { notify({ kind: "info", text: "This memory has no backend identifier and cannot be deleted from the live store." }); return; }
    try {
      await api.deleteMemory(memory.id);
      setMemories((current) => current.filter((item) => item.id !== memory.id));
      notify({ kind: "success", text: "Memory deleted from the live workspace." });
    } catch {
      notify({ kind: "error", text: "David could not delete that memory from the live service." });
    }
  }

  return <div><PageHeader route="memory" action={<button className="button button-primary" onClick={() => void addMemory()} disabled={working}><Plus size={16} /> {working ? "Saving..." : "Add memory"}</button>} /><div className="memory-overview"><div className="memory-score"><div className="score-ring"><strong>{loading ? "—" : memories.length}</strong><span>live</span></div><div><span className="micro-label">LIVE CONTEXT</span><h2>{loading ? "Loading David's memory." : memories.length ? "David's workspace context is available." : "David has no stored memories yet."}</h2><p>These records come from the connected memory service. Nothing is presented as indexed until the backend returns it.</p></div></div><div className="memory-facts"><div><strong>{loading ? "—" : memories.length}</strong><span>records</span></div><div><strong>{query ? "1" : "—"}</strong><span>active query</span></div><div><strong>API</strong><span>source</span></div></div></div><div className="filter-row"><div className="filter-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void search(); }} placeholder="Search live memories" /></div><button className="filter-button" onClick={() => void search()} disabled={searching}>{searching ? "Searching..." : "Search memory"} <Search size={14} /></button></div>{loading ? <div className="panel-card padded-card"><span className="micro-label">LOADING LIVE MEMORY</span></div> : !memories.length ? <div className="panel-card padded-card"><span className="micro-label">NO LIVE MEMORIES</span><p>David found no stored memory records for this workspace.</p></div> : <div className="memory-grid">{memories.map((memory) => <article className="panel-card memory-card" key={memory.id || memory.content}><div className="memory-card-top"><span className="memory-icon"><BrainCircuit size={17} /></span><span className="status-tag tag-blue">{memory.type || "Memory"}</span><button className="icon-button" onClick={() => void removeMemory(memory)} aria-label={`Delete memory: ${memory.content}`}><Trash2 size={15} /></button></div><h3>{memory.content.slice(0, 72)}{memory.content.length > 72 ? "…" : ""}</h3><p>{memory.source || "Live workspace memory"}</p><div className="memory-card-footer"><span>{memory.tags?.join(" · ") || "Stored by David"}</span></div></article>)}</div>}</div>;
}
function TemplatesView({ onRunObjective, notify }: { onRunObjective: (objective: string) => void; notify: (toast: Toast) => void }) {
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const selected = davidTemplates.find((template) => template.id === selectedId) || null;
  function runTemplate(template: DavidTemplate) {
    setSelectedId(template.id);
    notify({ kind: "info", text: `Running ${template.name} through David's live governed execution path.` });
    onRunObjective(template.objective);
  }
  return <div className="templates-page"><PageHeader route="templates" action={<span className="status-tag tag-blue"><span className="status-dot" /> {davidTemplates.length} live templates</span>} /><section className="template-hero panel-card"><div><div className="eyebrow-pill">ACCESSIBLE WORKFLOW LIBRARY</div><h2>Start from a real operating pattern.</h2><p>Each template is a structured objective. Choosing one sends it through David’s live planner, policy checks, capability routing, execution, and verification path.</p></div><div className="template-hero-mark"><Library size={28} /><span>REAL RUNS</span></div></section><div className="template-grid">{davidTemplates.map((template) => <article className={cx("panel-card template-card", selectedId === template.id && "template-card-selected")} key={template.id}><div className="template-card-top"><span className="status-tag tag-blue">{template.category}</span>{template.approvalRequired && <span className="status-tag tag-amber">Approval may be required</span>}</div><h3>{template.name}</h3><p>{template.description}</p><ol className="template-steps">{template.steps.map((step) => <li key={step}>{step}</li>)}</ol><button className="button button-primary wide-button" onClick={() => runTemplate(template)} aria-label={`Run ${template.name} template`}>{selectedId === template.id ? "Running through David..." : "Run template"}<ArrowUpRight size={14} /></button></article>)}</div>{selected && <div className="panel-card template-live-panel" role="status" aria-live="polite"><span className="micro-label">LIVE TEMPLATE OBJECTIVE</span><h3>{selected.name}</h3><p>{selected.objective}</p><span>Execution status is visible in the Overview timeline.</span></div>}</div>;
}

function CreativeView({ onNavigate, notify }: { onNavigate: (route: RouteKey) => void; notify: (toast: Toast) => void }) { const tools = [{ title: "Website builder", detail: "Brief to responsive, on-brand site.", icon: Globe2, route: "website-builder" as RouteKey, color: "red" }, { title: "Video studio", detail: "Script, scenes, voice, and social cuts.", icon: Film, route: "video-studio" as RouteKey, color: "purple" }, { title: "Voice studio", detail: "Natural narration and real-time conversation.", icon: AudioLines, route: "voice-studio" as RouteKey, color: "blue" }, { title: "Image lab", detail: "Campaign visuals, thumbnails, and diagrams.", icon: ImageIcon, route: "image-studio" as RouteKey, color: "amber" }, { title: "Document forge", detail: "Reports, proposals, briefs, and decks.", icon: FileText, route: "creative" as RouteKey, color: "green" }, { title: "Asset library", detail: "A searchable home for every generated artifact.", icon: Library, route: "files" as RouteKey, color: "red" }]; return <div><PageHeader route="creative" action={<button className="button button-primary" onClick={() => notify({ kind: "success", text: "New creative workspace created." })}><Plus size={16} /> New creation</button>} /><div className="creative-hero panel-card"><div><div className="eyebrow-pill">CONNECTED PRODUCTION</div><h2>One brief. Every format.</h2><p>David can turn a product idea into the pages, images, videos, voiceovers, and documents your team needs.</p></div><div className="creative-pipeline"><span>Brief</span><ChevronRight size={15} /><span>Plan</span><ChevronRight size={15} /><span>Generate</span><ChevronRight size={15} /><span>Review</span></div></div><div className="creative-grid">{tools.map((tool) => { const Icon = tool.icon; return <button className="panel-card creative-card" key={tool.title} onClick={() => onNavigate(tool.route)}><div className={cx("creative-icon", `tone-${tool.color}`)}><Icon size={21} /></div><div><h3>{tool.title}</h3><p>{tool.detail}</p></div><ArrowUpRight size={16} className="card-arrow" /></button>; })}</div></div>; }

function WebsiteBuilder({ notify }: { notify: (toast: Toast) => void }) { const [brief, setBrief] = useState("A conversion-focused launch page for Atlas, a calm operating system for independent business owners."); const [building, setBuilding] = useState(false); const [result, setResult] = useState<Record<string, unknown> | null>(null); const [error, setError] = useState(""); async function buildWebsite() { if (!brief.trim() || building) return; setBuilding(true); setError(""); setResult(null); try { const response = await api.websiteGenerate(brief); setResult(response); notify({ kind: "success", text: "Website generation returned a real result for review." }); } catch (cause) { const message = cause instanceof Error ? cause.message : "Website generation is unavailable."; setError(message); notify({ kind: "error", text: "No website was generated. Check the backend and provider configuration." }); } finally { setBuilding(false); } } async function saveBrief() { try { await api.planCreate(brief); notify({ kind: "success", text: "Build brief saved as a live plan." }); } catch { notify({ kind: "error", text: "The build brief could not be saved because the planning service is unavailable." }); } } return <div><PageHeader route="website-builder" action={<div className="header-actions"><button className="button button-secondary" onClick={() => { const html = typeof result?.html === "string" ? result.html : ""; if (!html) { notify({ kind: "info", text: "Generate a real website result before opening a preview." }); return; } const url = URL.createObjectURL(new Blob([html], { type: "text/html" })); window.open(url, "_blank", "noopener,noreferrer"); window.setTimeout(() => URL.revokeObjectURL(url), 60000); }}><Globe2 size={16} /> Preview</button><button className="button button-primary" onClick={() => void buildWebsite()} disabled={building || !brief.trim()}><Rocket size={16} /> {building ? "Calling website service..." : "Build website"}</button></div>} /><div className="builder-layout"><section className="panel-card builder-prompt"><SectionHeader eyebrow="BUILD BRIEF" title="Tell David what to build" detail="David will propose the structure, copy, visual direction, and implementation plan." icon={WandSparkles} /><textarea value={brief} onChange={(event) => setBrief(event.target.value)} /><div className="builder-options"><button className="option-chip active"><Palette size={14} /> Brand-aware</button><button className="option-chip"><Globe2 size={14} /> Responsive</button><button className="option-chip"><ShieldCheck size={14} /> Review before publish</button></div><div className="builder-action-row"><span><LockKeyhole size={14} /> Publishing always requires approval</span><button className="button button-primary" onClick={() => void saveBrief()}><Check size={15} /> Save brief</button></div>{error && <div className="provider-result"><span className="micro-label">WEBSITE RESULT</span><p>Generation unavailable: {error}</p></div>}{result && <div className="provider-result"><span className="micro-label">WEBSITE RESULT</span><p>Backend returned a reviewable website payload. Preview it only when an HTML artifact is present.</p></div>}</section><section className="panel-card browser-preview"><div className="browser-top"><div className="browser-dots"><span /><span /><span /></div><span>atlas.david.ai / preview</span><MoreHorizontal size={17} /></div><div className="preview-content"><div className="preview-nav"><strong>ATLAS</strong><span>Product</span><span>How it works</span><span>Pricing</span><button>Start free</button></div><div className="preview-hero"><span className="eyebrow-pill">A CALMER WAY TO OPERATE</span><h2>Your business, with more leverage.</h2><p>Bring your plans, systems, and next best actions into one intelligent workspace.</p><button><Sparkles size={14} /> Explore Atlas</button></div><div className="preview-blocks"><div /><div /><div /></div></div></section></div></div>; }

function VideoStudio({ notify }: { notify: (toast: Toast) => void }) {
  const [prompt, setPrompt] = useState("An 8-second cinematic David AI product teaser. Dark cyan HUD, restrained orbital light, confident pacing, premium founder-focused tone, 16:9 landscape.");
  const [working, setWorking] = useState(false);
  const [result, setResult] = useState<{ status: string; message: string; url?: string }>({ status: "ready", message: "Ready for a real Gemini video operation." });

  async function renderVideo() {
    if (!prompt.trim() || working) return;
    setWorking(true);
    setResult({ status: "processing", message: "David has started the verified video operation. This page will keep polling until the MP4 is ready." });
    try {
      const started = await api.providers.execute("video", { prompt, aspect_ratio: "16:9" }, ["gemini"]);
      const operationName = typeof started.operation_name === "string" ? started.operation_name : "";
      if (!operationName) throw new Error("The video provider did not return a trackable operation.");
      let completed: Record<string, unknown> | null = null;
      for (let attempt = 0; attempt < 60; attempt += 1) {
        await new Promise((resolve) => window.setTimeout(resolve, attempt === 0 ? 2500 : 5000));
        const status = await api.providers.videoOperation(operationName);
        if (status.status === "completed" && typeof status.video_uri === "string") { completed = status; break; }
        if (status.status !== "processing") throw new Error(String(status.message || "Video generation did not complete."));
        setResult({ status: "processing", message: `Video generation is still processing. Poll ${attempt + 1} of 60.` });
      }
      if (!completed || typeof completed.video_uri !== "string") throw new Error("The video provider did not finish within the active generation window.");
      const url = api.providers.videoDownloadUrl(completed.video_uri);
      setResult({ status: "complete", message: "Verified MP4 video artifact ready for playback and download.", url });
      notify({ kind: "success", text: "Verified David AI video artifact is ready." });
    } catch (error) {
      const message = error instanceof Error ? error.message : "Video generation failed.";
      setResult({ status: "error", message });
      notify({ kind: "error", text: "David did not mark the video complete because no verified artifact was returned." });
    } finally {
      setWorking(false);
    }
  }

  const complete = result.status === "complete" && result.url;
  return <div><PageHeader route="video-studio" action={<button className="button button-primary" onClick={() => void renderVideo()} disabled={working || !prompt.trim()}><Play size={16} /> {working ? "Generating…" : "Generate video"}</button>} /><div className="video-studio-layout"><section className="panel-card video-prompt-card"><SectionHeader eyebrow="MODEL-BACKED PRODUCTION" title="Shape the next cut" icon={Film} /><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} /><div className="video-controls"><div><span className="micro-label">FORMAT</span><span className="select-like">Landscape 16:9</span></div><div><span className="micro-label">MODEL</span><span className="select-like">Gemini / Veo</span></div><div><span className="micro-label">OUTPUT</span><span className="select-like">Verified MP4</span></div></div><button className="button button-primary wide-button" onClick={() => void renderVideo()} disabled={working || !prompt.trim()}><WandSparkles size={16} /> {working ? "Generating video" : "Generate now"}</button><div className={cx("provider-result", result.status === "error" && "provider-result-error")}><span className="micro-label">VIDEO STATUS</span><p>{result.message}</p>{complete && <div className="media-result-actions"><a className="button button-secondary" href={result.url} target="_blank" rel="noreferrer"><Play size={15} /> Open video</a><a className="button button-secondary" href={result.url} download><Download size={15} /> Download MP4</a></div>}</div></section><section className="panel-card timeline-card"><div className="timeline-header"><div><div className="micro-label">VERIFIED RENDER STATE</div><h2>{complete ? "Video ready" : "No fake completion"}</h2></div><span className={cx("status-tag", working ? "tag-amber" : result.status === "complete" ? "tag-green" : result.status === "error" ? "tag-red" : "tag-blue")}>{working ? "Processing" : result.status === "complete" ? "Verified artifact" : result.status === "error" ? "Needs review" : "Ready"}</span></div>{complete ? <video className="video-result-player" controls playsInline src={result.url} /> : <div className="video-canvas"><div className="canvas-orbit" /><div className="canvas-brand">DAVID AI</div><div className="canvas-copy">Model-backed.<br /><span>Reviewable.</span></div><div className="canvas-caption">The preview updates only after a real completed operation.</div></div>}<div className="timeline-track"><div className="timeline-row"><span className="track-label"><Film size={13} /> Gemini operation</span><div className="track-bar"><i className={working ? "active" : complete ? "active complete" : ""} /><i /><i /></div></div><div className="timeline-row"><span className="track-label"><AudioLines size={13} /> Native audio</span><div className="voice-wave"><span /><span /><span /><span /><span /><span /><span /><span /><span /><span /><span /><span /></div></div></div></section></div></div>;
}
...
function ProvidersView({ notify }: { notify: (toast: Toast) => void }) {
  ...
  async function loadProviders() {
    ...
    try {
      ...
      notify({ kind: "error", text: "Provider registry is unavailable. David did not invent health results." });
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const readiness = providers.length ? Math.round((configuredCount / providers.length) * 100) : 0;
  const iconFor = (provider: ProviderStatus): LucideIcon => provider.category === "voice" ? AudioLines : provider.category === "video" ? Film : provider.category === "deployment" ? Rocket : provider.category === "website" ? Globe2 : provider.category === "embeddings" ? Database : Sparkles;
  const toneFor = (provider: ProviderStatus) => provider.configured && provider.status === "supported" ? "green" : provider.configured ? "amber" : "blue";
  const labelFor = (provider: ProviderStatus) => provider.configured ? (provider.status === "supported" ? "Configured" : String(provider.status || "Configured")) : "Not configured";
