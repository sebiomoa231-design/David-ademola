...
import { DavidCore } from "@/components/DavidCore";
...
function OrchestrationDiagnostics({ state, summary, plan }: { state: OperatorCoreState; summary: string; plan: PersistedPlan | null }) {
  const active = ["thinking", "processing", "planning", "executing", "generating", "verifying", "speaking"].includes(state);
  if (!active) return null;
  const stageLabel = state === "thinking" ? "ANALYSING REQUEST" : state === "processing" ? "PROCESSING INPUT" : state === "planning" ? "ASSEMBLING PLAN" : state === "executing" ? "COMPOSING RESPONSE" : state === "generating" ? "GENERATING OUTPUT" : state === "verifying" ? "VERIFYING DELIVERY" : "VOICE RESPONSE";
...
export default function Home() {
  ...
  const voice = useOperatorVoice({
    ...
    synthesize: async (text) => {
      ...
      if (result.state !== "ready" || typeof audioBase64 !== "string") throw new Error(result.message || "David AI voice output is unavailable.");
  ...
  useEffect(() => {
    ...
    if (voice.state === "transcribing") { setCoreState("processing"); setLifecycleSummary("TRANSCRIBING — SENDING AUDIO TO DAVID AI OPERATOR"); }
    if (voice.state === "reasoning") { setCoreState("thinking"); setLifecycleSummary("ANALYSING REQUEST — PREPARING VERIFIED RESPONSE"); }
    if (voice.state === "speaking") { setCoreState("speaking"); setLifecycleSummary("DAVID VOICE OUTPUT ACTIVE"); }
    if (voice.state === "paused") { setCoreState("waiting"); setLifecycleSummary("VOICE OUTPUT PAUSED — READY TO RESUME"); }
    if (voice.state === "degraded") { setCoreState("degraded"); setLifecycleSummary("VOICE SERVICE NEEDS ATTENTION"); }
    if (voice.state === "cancelled") { setCoreState("idle"); setLifecycleSummary("VOICE OUTPUT INTERRUPTED — STANDING BY"); }
    if (voice.state === "idle" && !isResponding) { setCoreState("idle"); setLifecycleSummary("DAVID IS AWAKE AND READY"); }
  }, [voice.state, isResponding]);
